...
def gen_burst(
    ...
    j_up = jitter * upscale # 64
    h_up = height * upscale # 512
    w_up = width * upscale #512
    channels = tf.shape(patches)[-1] # 1 or 3

    bigj_patches = patches
    delta_up = (jitter - small_jitter) * upscale # (16-2)*4
    smallj_patches = patches[:, delta_up:-delta_up, delta_up:-delta_up, ...]
    # size of smallj_p: 528*528
    ...
    for i in range(unique):
        for j in range(repeats):
            ...
            for k in range(burst_length - 1):
                ...
                # if flip < prob use bigj_patches otherwise use smallj_patches
                p2use = tf.cond(
...
def load_patches(
    ...
    j_up = jitter * upscale # jitter=16 upscale=4 ->j_up=64
    h_up = height * upscale + 2 * j_up # 512+128->640
    w_up = width * upscale + 2 * j_up
    #pad if h_up/w_up larger than size of img
    ...
    if not is_val:
        ...
        # extract patches with sliding window size=640 and stride=320
